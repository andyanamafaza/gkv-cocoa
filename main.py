...
df = pd.read_csv("Area-kopi.csv")
df = df.drop(columns=["No"])

df_reshaped = df.melt(
    id_vars=["Provinsi"],
    var_name="Year",
    value_name="Luas Areal Kopi (Ha)"
)
...
app.layout = html.Div([
    html.H1(
        children='Visualisasi Data Luas Areal Kopi di Indonesia',
        style={
            'textAlign': 'center',
            'color': '#38220F'
        }
    ...
    html.Div([
        ...
        dcc.Graph(
            ...
            figure=px.choropleth(
                ...
                color="Luas Areal Kopi (Ha)",
                color_continuous_scale=colorscale,
                labels={"Luas Areal Kopi (Ha)": "Luas Areal Kopi (Ha)"},
            ).update_geos(fitbounds="locations", visible=False).update_traces(
                hovertemplate="<b>%{location}</b><br>Luas Areal Kopi: %{z} Ha"
            ),
